...
# Sample DataFrame
# This DataFrame represents a sample dataset of 10 records, where each record
# contains an 'Age' column and a 'Price' column. The 'Age' column has values
# ranging from 22 to 50, and the 'Price' column has values ranging from 700 to
# 2000. This dataset is used to demonstrate a simple linear regression model.
...
# Linear regression
# The np.polyfit() function is used to fit a 1st degree polynomial to the data,
# which is equivalent to a linear regression model. The function returns the
# coefficients of the polynomial, which are used to calculate the intercept and
# slope of the regression line. The intercept is the value of y when x is 0, and
# the slope is the change in y over the change in x. The slope is the change in
# the dependent variable (Price) per unit change in the independent variable
# (Age). The intercept is the value of the dependent variable when the
